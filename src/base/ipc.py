...
class IPCChannel:
    ...
    def _handle_auth_packet(self, seq, prefix, command, payload):
        ...
        if prefix == "req":
            # We've received a challenge.  If we've already sent a response
            # to a previous challenge (which is the case if _pending_challenge
            # is not None), then something isn't right.  This could be a DoS
            # so we'll disconnect immediately.
            ...
            # We are expecting a response to our previous challenge.  First
            # compute the response we expect to have received based on the
            # challenge sent earlier, our shared secret, and the salt that was
            # generated by the remote end.
            ...
            # If remote has issued a challenge along with their response, we'll
            # respond.  Unless something fishy is going on, this should always
            # succeed on the remote end, because at this point our auth secrets
            # must match.  A challenge is considered issued if it is not NULL
            # ('\x00' * 20).  If no challenge is returned in step 3 (above),
            # then authentication is only one-sided (we trust the remote, but
            # the remote won't trust us).  In this case, things won't work
